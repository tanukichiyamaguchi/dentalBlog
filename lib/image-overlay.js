...
function buildSvgOverlay(titleLines) {
  ...
  const titleElements = titleLines
    .map((line, i) => {
      ...
      return `<text x="${OGP_WIDTH / 2}" y="${y}" text-anchor="middle" font-family="'Noto Sans CJK JP', 'Meiryo', sans-serif" font-size="${fontSize}" font-weight="bold" fill="white">${escapeXml(line)}</text>`;
    })
    .join('\n    ');
