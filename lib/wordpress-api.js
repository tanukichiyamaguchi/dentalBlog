...
async function wpRequest(method, endpoint, body = null) {
  ...
  // Use ?rest_route= instead of /wp-json/ to bypass XSERVER REST API path restriction
  // Split endpoint into path and query parts so query params become & separated
  const [epPath, epQuery] = endpoint.split('?');
  let url = `${WP_SITE_URL}/?rest_route=${epPath}`;
  if (epQuery) {
    url += `&${epQuery}`;
  }
...
async function uploadMedia(filePath, altText = '') {
  ...
  const url = `${WP_SITE_URL}/?rest_route=/wp/v2/media`;

  const mediaAuth = authHeader();
  const headers = {
    Authorization: mediaAuth,
    'X-WP-Authorization': mediaAuth,
